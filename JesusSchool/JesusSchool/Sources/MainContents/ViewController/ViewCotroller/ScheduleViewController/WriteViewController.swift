...
class WriteViewController: UIViewController, UITextViewDelegate {
  ...
  override func viewDidLoad() {
    ...
    selectedDate.layer.borderColor = #colorLiteral(red: 0.6666666667, green: 0.6666666667, blue: 0.6666666667, alpha: 1)
    selectedDate.layer.borderWidth = 1 / UIScreen.main.scale
    selectedDate.layer.cornerRadius = 22

    
    scheduleTextView.layer.borderColor = #colorLiteral(red: 0.6666666667, green: 0.6666666667, blue: 0.6666666667, alpha: 1)
  ...
  @IBAction func didTabCancelBtn(_ sender: UIButton) {
    if scheduleTextView.text != nil {
      ...
      let alretCancelAction = UIAlertAction(title: "취소", style: .default, handler: nil)
      let alretAction = UIAlertAction(title: "확인", style: .cancel, handler: { (action) in
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    if segue.identifier == "unwindSegueWriteToSchedule" {
      if let vc = segue.destination as? ScheduleViewController, let scheduleDatas = sender as? [ScheduleData] {
